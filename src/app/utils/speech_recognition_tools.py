"""Speech recognition and synthesis tools for audio processing."""
...
def int2float(sound: npt.NDArray[np.int16]) -> npt.NDArray[np.float32]:
    """Convert int16 audio samples to float32 normalized between -1 and 1."""
    abs_max = np.abs(sound).max()
    sound_32: npt.NDArray[np.float32] = sound.astype(np.float32)
    if abs_max > 0:
        sound_32 *= 1 / 32768
...
async def send_audio_to_stt_api(
    ...
    try:
        ...
        logger.error("Request timed out after 10.0 seconds")
