...
class TestWebSocketHandling:
    ...
    async def test_setup_satellite_connection(self, mock_client_config, mock_processor, 
                                            mock_websocket, mock_sup_util):
        ...
        mock_sup_util.mqtt_client.subscribe.assert_called_once_with(
...
class TestWebSocketEndpoint:
    ...
    async def test_websocket_endpoint_duplicate_connection(self, mock_sup_util, mock_websocket):
        ...
        mock_websocket.close.assert_called_once_with(
            code=1001, reason="Connection already exists"
        )
